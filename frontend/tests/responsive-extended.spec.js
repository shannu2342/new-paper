  ...
  for (const route of routes) {
    test(`${vp.name} ${route}`, async ({ page }) => {
      ...
      const scan = async () => page.evaluate(() => {
        ...
        return { overflow, vw, sw: document.documentElement.scrollWidth, offenders, path: location.pathname };
      });

      const before = await scan();
      console.log(JSON.stringify({ viewport: vp.name, route, state: 'default', ...before }));

      if (vp.name === 'mobile' && !route.startsWith('/admin')) {
      ...
      await page.screenshot({ path: `/tmp/responsive-ext-${vp.name}-${route.replace(/\//g, '_') || 'home'}.png`, fullPage: true });
    });
  }
}
