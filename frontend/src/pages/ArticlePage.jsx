...
const ArticlePage = () => {
  ...
  const content = article.content?.[language] || article.content?.en || article.content?.te || article.content?.hi;

  return (
    <main className="page">
      <article className="article-detail">
        <h1>{title}</h1>
        <div className="article-meta">{formatArticleDateLabel(article.dateKey, language)}</div>
        {article.images?.length ? (
          <div className={`article-gallery${article.images.length === 1 ? ' article-gallery--single' : ''}`}>
            {article.images.map((img) => (
              <figure key={img} className="article-gallery__item">
                <img
                  key={img}
                  src={img}
                  alt={title}
