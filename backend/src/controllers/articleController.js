...
const sanitizeBilingualContent = (value = {}) => ({
  ...
  hi: sanitizeHtml(value.hi)
});

const normalizeOptionalBilingual = (value) => {
  if (!value) return undefined;
  const normalized = {
    en: String(value.en || '').trim(),
    te: String(value.te || '').trim(),
    hi: String(value.hi || '').trim()
  };
  if (!normalized.en && !normalized.te && !normalized.hi) {
    return undefined;
  }
  return normalized;
};
...
const listArticles = async (req, res) => {
  const { date, categoryType, apRegion, district, category, otherCategoryKey, isBreaking, isFeatured, limit, status, q, beforeOrOn } = req.query;
  const { partition, districtCode } = req.query;
  const query = {};
  if (date) {
    query.dateKey = beforeOrOn === 'true' ? { $lte: toDateKey(date) } : toDateKey(date);
  }
  if (categoryType) {
...
const createArticle = async (req, res) => {
  try {
    ...
    payload.summary = normalizeOptionalBilingual(sanitizeBilingualContent(payload.summary || {}));
    payload.otherCategory = normalizeOptionalBilingual(payload.otherCategory);
    if (payload.categoryType !== 'other') {
      payload.otherCategory = undefined;
      if (payload.categoryType !== 'special') {
        payload.otherCategoryKey = '';
      }
    }
    ...
    const message = error?.name === 'ValidationError' ? error.message : 'Unable to create article';
...
const updateArticle = async (req, res) => {
  try {
    ...
    if (Object.prototype.hasOwnProperty.call(payload, 'otherCategory')) {
      payload.otherCategory = normalizeOptionalBilingual(payload.otherCategory);
    }
    if (payload.categoryType && payload.categoryType !== 'other') {
      payload.otherCategory = undefined;
      if (payload.categoryType !== 'special') {
        payload.otherCategoryKey = '';
      }
    ...
    const article = await Article.findByIdAndUpdate(id, payload, { new: true, runValidators: true });
    if (!article) {
      return res.status(404).json({ message: 'Article not found' });
    }
    return res.json(article);
  } catch (error) {
    const message = error?.name === 'ValidationError' ? error.message : 'Unable to update article';
    return res.status(400).json({ message });
  }
};
